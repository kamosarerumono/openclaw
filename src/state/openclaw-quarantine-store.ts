...
/** Refresh verification without turning a running writer into a clean close. */
...
/** A completed background read cannot undo an intervening unclean release. */
export function refreshOpenClawAgentIntegrityVerification(
  pathname: string,
  env: NodeJS.ProcessEnv,
  identity: string,
): void {
  ...
  withQuarantineWriter(env, (database) => {
    executeSqliteQuerySync(
      database,
      getNodeSqliteKysely<IntegrityDatabase>(database)
        .updateTable("agent_integrity_verifications")
