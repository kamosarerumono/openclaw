...
describe("OpenClaw database integrity verifier", () => {
  ...
  it.each(["present", "cleared"] as const)(
    "refreshes quick-check proof only while its existing receipt is %s",
    async (receipt) => {
      ...
      const results = await runDatabaseVerifyWorker(targets);
      expect(results).toEqual([
        { path: agent.path, ok: true, identity: `${before?.dev}:${before?.ino}` },
      ]);
      if (receipt === "cleared") {
      ...
      expect(readOpenClawAgentIntegrityVerification(agent.path, env)).toEqual(
        receipt === "cleared" ? undefined : { ...before, verified_at: verifiedAt },
      );
    },
  );

  it("still quarantines failed targets when an earlier healthy receipt cannot refresh", async () => {
    const stateDir = tempDirs.make("openclaw-database-quick-refresh-failure-");
    const env = { OPENCLAW_STATE_DIR: stateDir };
    const healthy = openOpenClawAgentDatabase({ agentId: "healthy", env });
    const damaged = openOpenClawAgentDatabase({ agentId: "damaged", env });
    const record = readOpenClawAgentIntegrityVerification(healthy.path, env);
    expect(record).toBeDefined();
    createUnsafeIndexDrift(damaged.path);
    const metadata = new (requireNodeSqlite().DatabaseSync)(quarantineStorePath(stateDir));
    try {
      metadata.exec(`CREATE TRIGGER reject_refresh BEFORE UPDATE OF verified_at ON agent_integrity_verifications
        BEGIN SELECT RAISE(FAIL, 'synthetic verification refresh failure'); END;`);
    } finally {
      metadata.close();
    }
    const targets: OpenClawDatabaseVerifyTarget[] = [
      { kind: "agent", label: "healthy agent", path: healthy.path, check: "quick" },
      { kind: "agent", label: "damaged agent", path: damaged.path, check: "quick" },
    ];
    await expect(
      applyOpenClawDatabaseVerificationResults({
        env,
        targets,
        results: [
          { path: healthy.path, ok: true, identity: `${record?.dev}:${record?.ino}` },
          terminalVerificationResult(damaged.path),
        ],
      }),
    ).resolves.toBeUndefined();
    expect(readOpenClawDatabaseQuarantine(damaged.path, { env })?.reason).toMatch(
